...
class Whiskey(db.Model):
    ...
    litres = db.Column(db.String(200))
    country = db.Column(db.String(20))
    user_token = db.Column(db.String, db.ForeignKey('user.token'), nullable = False)

    def __init__(self, brand, litres,  country, user_token, id=''):
        self.id = self.set_id()
        self.brand = brand
        self.litres = litres
    ...
    def __repr__(self):
        return f'The following has been added to the Whiskeys: {self.brand}'
...
class WhiskeySchema(ma.Schema):
    class Meta:
        fields = ['id', 'brand', 'litres', 'country']
